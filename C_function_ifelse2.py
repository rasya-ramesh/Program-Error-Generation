...
tokens = [ 'COLON', 'COMMA','TAB','NUMBER','NAME','GREATERTHAN','LESSTHAN','LESSEQ','GREATEQ','LPAREN','RPAREN', 'LFPAREN', 'RFPAREN', 'SCOLON', 'TIMES','DIVIDES', 'PLUS', 'MINUS', 'EQUALS','TIMES'] + list(reserved.values())
...
def t_LESSEQ(t):
  r'\<='
  t.value = Node("LESSEQ", '<=', leaf = 1)
  return t

def t_GREATEQ(t):
  r'\>='
  t.value = Node("GREATEQ", '>=', leaf = 1)
  return t
...
def t_NUMBER(t):
     r'\d+'
     t.type = reserved.get(t.value,'NUMBER')    # Check for reserved words
...
def p_iterationStmt(t):
	...
	'''iterationStmt : WHILE LPAREN simpleexpression RPAREN LFPAREN statement RFPAREN
					  | FOR LPAREN varDeclaration SCOLON forcondition SCOLON forchange RPAREN LFPAREN statement RFPAREN'''
